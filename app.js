...
// ---- Reminder timestamps & catch-up helpers ----
...
function toggleTask(q,id){const t=state.tasks[q].find(x=>x.id===id);if(!t)return;if(!t.done){if(!confirm('Confermi completamento? Evita la procrastinazione.'))return;t.done=true;state.userStats.currentXP+=50;state.userStats.weeklyCompleted+=1;state.userStats.totalCompleted+=1;if(state.userStats.currentXP>=state.userStats.nextLevelXP){state.userStats.level+=1;state.userStats.currentXP-=state.userStats.nextLevelXP;state.userStats.nextLevelXP=Math.floor(state.userStats.nextLevelXP*1.5);toast(`Level up! Lv ${state.userStats.level}`)}else{toast('Task completato (+50 XP)')}}else{t.done=false}saveState();renderAll()}
...
// Health
$('#waterBanner').addEventListener('click',()=>{state.userStats.waterToday+=1;saveState();toast('💧 Fatto!')});
$('#bathBanner').addEventListener('click',()=>{state.userStats.bathroomToday+=1;saveState();toast('🚻 Fatto!')});

// Notifications every 6h (with timestamps)
...
function runFrequentRemindersOnce(){
  ...
  if(hasQ1){
    if(minutesSince('focusly_last_q1')>=REMINDERS.q1Minutes){
